...
my.reads.trans$type<-ifelse(my.reads.trans$nest=="neg1","Negative",
                            ifelse(my.reads.trans$nest=="neg2","Negative",
                                   ifelse(my.reads.trans$nest=="neg1R","Negative",
                                          ifelse(my.reads.trans$nest=="neg2R","Negative",
                                              ifelse(my.reads.trans$nest=="DNApositive","DNApositive",
                                                  ifelse(my.reads.trans$nest=="DNApositiveR","DNApositive",
                                                        ifelse(my.reads.trans$nest=="PCRpositiveR","PCRpositive",
...
### total all the reads
my.reads.trans$total<-rowSums(my.reads.trans[c(2:5)])
...
my.reads.trans[,2:5][my.reads.trans[,2:5] < cutoff] <- 0

### subset the data frame to drop all coloumns containing only zeros
my.reads.trans.nozero<-cbind(my.reads.trans[,c(1, 6:12)], subset(my.reads.trans[,c(2:5)], select = colSums(my.reads.trans[,c(2:5)]) !=0))

### remove the total variable
my.reads.trans.drop<-my.reads.trans.nozero[,c(1:6,8:12)]
...
my.reads.trans.drop$panel<-as.factor(c(rep(1,times=384),
                                       rep(2,times=384),
                                       rep(3,times=383)))
...
hit.hist<-data.frame(OTUs = rowSums(my.reads.trans.samps.only[c(2,8:11)] != 0), Type=my.reads.trans.samps.only$type)
...
well.composition<-ggplot(data=my.reads.melt, aes(x=Sample, y=Reads, fill=Species)) +
  ...
  theme(axis.text.x = element_text(size = rel(0.2), angle =90),
